...
# 3. Decision tree를 사용하여 학습 진행 (using X_train,y_train)
from sklearn import tree

dtree = tree.DecisionTreeClassifier()
dtree_model = dtree.fit(X_train,y_train)

# 4. predict 함수에 X_test data를 사용하여 y_predict를 도출 
...
# 3. K-fold에서 추출한 train과 test index를 기준으로 X,y 각각의 train,test data를 설정해준다. kf.split(X)는 n_splits만큼의 경우를 갖고있다.
